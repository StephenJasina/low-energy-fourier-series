...
void sgd_step(FourierSeries &series, vector<pair<int, int> > &sum_keys,
              ...
              long double p, long double eta_divisor = 1) {
  long double eta;
...
void gd(FourierSeries &series, long double h, long double p,
  ...
  e = series.e_stretch() + series.e_mat() + h * h * series.e_bend() +
  ...
  while (e > max_e && eta_divisor < 10000) {
    ...
    gd_step(series, sum_keys, half_keys, h, p, eta_divisor);
...
int main(int argc, char **argv) {
  ...
  gd(series, h, p, true, max_e, 10, 1.1);
